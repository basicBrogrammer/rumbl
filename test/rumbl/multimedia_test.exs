defmodule Rumbl.MultimediaTest do
  ...
  describe "videos" do
    ...
    test "get_video!/1 returns the video with a given id" do
      owner = user_fixture()
      %Video{id: id, title: title} = video_fixture(owner)
      assert %Video{id: ^id, title: ^title} = Multimedia.get_video!(id)
    end
    ...
    test "create_video/2 with invalid data returns an error" do
      owner = user_fixture()
      assert {:error, %Ecto.Changeset{} = video} = Multimedia.create_video(owner, @invalid_attrs)
    end
